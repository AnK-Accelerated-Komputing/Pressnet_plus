...
def compute_metrics(pred, gt):
    """
    CORRECTED: Compute proper element-wise metrics
    """
    error = pred - gt  # Element-wise error
    l2 = torch.norm(error, p=2)  # L2 norm of error vector
    rmse = torch.sqrt(torch.mean(error ** 2))  # Root mean squared error
    max_rmse = torch.sqrt((torch.max(pred) - torch.max(gt)) ** 2)  # Absolute difference of maxima
...
def plot_domain_wise_loss(x_ranges,results, output_dir,key='all'):
    ...
    for key in keys:
        ...
        plt.xticks(
            x_range_labels, 
            # Swapped x_min and x_max for standard reading order [min, max]
            [f"[{x_min:.1f}, {x_max:.1f}]" for x_min, x_max in results['range']], 
            fontsize=10,    # Changed from 1 to 10
            rotation=90     # Rotated to prevent overlapping
        )
        # plt.ylim(0, max_error+0.1)
        plt.ylim(0, max(errors) * 1.2)
        plt.xlim(min(x_range_labels) - 20, max(x_range_labels) + 20)
...
def evaluate_rollout(rollout_pth, base_output_dir,plot=True):
    ...
    for i in range(len(rollout)):
        ...
        avg_error_path = os.path.join(output_dir, 'Average_deformation_error.pkl')
        max_error_path = os.path.join(output_dir, 'Max_deformation_error.pkl')
    
        with open(avg_error_path, 'wb') as file:
            pickle.dump(average_deformation_error, file)

        with open(max_error_path, 'wb') as file:
...
def main():
    base_dir = "/home/sushil/PressNet/_NAS_MOUNT/ank-server1/prj_accelerated_physics/prj_pressnet/Training_data_Susil/Server6/output/transolver/coarse_1500_train_val/Transolver_C_ST1_Slice16/rollout"
    rollout_pth = os.path.join(base_dir,'infer_rollout.pkl')
    output_dir = os.path.join(base_dir,'rollout_evaluation_transolver_1500_coarse')
